...
import focandlol.resproject.review.repository.ReviewRepository;
import focandlol.resproject.review.type.ReviewUpdateType;
...
import static focandlol.resproject.review.type.ReviewUpdateType.*;
...
public class ReviewServiceImpl implements ReviewService {
    ...
    private static void validateDeleteReview(CustomUserDetails user, List<String> roles, ReviewEntity review) {
        /**
         * role에 따라서 검증
         * 로그인한 사용자가 manager라면 가게 주인(매니저)과 현재 로그인한 사용자가 동일한지 확인
         * 로그인한 사용자가 customer라면 리뷰 작성자와 현재 로그인한 사용자가 동일한지 확인
         */
        if (roles.contains("ROLE_MANAGER")) {
            if (!user.getId().equals(review.getStore().getManager().getId())) {
                throw new CustomException(ANOTHER_MANAGER);
            }
        } else if (roles.contains("ROLE_CUSTOMER")) {
            if (!user.getId().equals(review.getCustomer().getId())) {
                throw new CustomException(ANOTHER_CUSTOMER);
            }
        }
    }
