...
import focandlol.resproject.reservation.repository.QueryReservationRepository;
import focandlol.resproject.reservation.repository.ReservationRepository;
import focandlol.resproject.store.entity.StoreEntity;
import focandlol.resproject.store.repository.StoreRepository;
...
import static focandlol.resproject.global.exception.ErrorCode.*;
import static focandlol.resproject.reservation.type.ReservationType.*;
...
public class ReservationServiceImpl implements ReservationService {
     ...
     *                              - storeName: 가게 이름으로 검색
     *                              - date: 특정 날짜로 검색
     *                              - sortBy: 정렬 기준 (STORE_NAME, DATE, TIME)
     *                              - reservationType: 예약 상태로 검색 (UNAPPROVED, APPROVED, VISITED, CANCELED)
     *                              - isAscending: 정렬 방향 (true: 오름차순, false: 내림차순)
     * @param pageable              페이징 정보 (페이지 크기 및 페이지 번호)
     *                              - querydsl 기반의 `queryReservationRepository`를 사용하여 동적 쿼리를 생성
     ...
     * @param pageable              페이징 정보 (페이지 크기 및 페이지 번호)
     *                              - querydsl 기반의 `queryReservationRepository`를 사용하여 동적 쿼리를 생성
     * @return 조건에 맞는 매니저가 관리하는 가게들의 예약 리스트
     */
    @Override
    ...
    /**
     * 현재 날짜를 기준으로 해당 달의 모든 날짜의 남은 예약 가능 인원수 리스트
     *
     * @param storeId : 예약을 조회할 가게 id
     * @return : 현재 날짜를 기준으로 해당 달의 모든 날짜의 남은 예약 가능 인원수 리스트
     */
    @Override
    public List<ReservationNumMonth> getReservationNumMonth(Long storeId) {
        /**
         * 리턴할 객체
         */
        List<ReservationNumMonth> list = new ArrayList<>();
        ...
        /**
         * 해당 가게의 현재 날짜 기준 해당 달의 모든 예약 조회
         * ex) 현재 날짜 2025-01-01 -> 2025년 1월달의 모든 예약 조회
         */
        List<ReservationEntity> reservation = reservationRepository.findByStoreIdAndDateRangeFetch(storeId, startDate, endDate);

        /**
         * 조회한 예약들을 날짜별로 그룹화, 날짜별 예약인원 합 리턴
         * ex) key : 2025-01-03, value : 4
         */
        Map<LocalDate, Integer> collect = reservation
                .stream()
                .collect(Collectors.groupingBy(
                        ReservationEntity::getDate, // 날짜별로 그룹화
                        Collectors.summingInt(ReservationEntity::getNumOfPeople) // numOfPeople 합계
                ));

        /**
         * 해당 달의 첫째날 부터 마지막 날 까지 순회
         * 만약 조회한 날짜에 예약이 있으면 가게의 총 예약 가능 인원수 - 현재 예약한 인원수
         * 예약이 없으면 store의 총 예약 가능 인원수
         * ReservationNumOfMonth.date : 날짜
         * ReservationNumOfMonth.leftSeat : 남은 예약 가능 인원수
         *
         * reservation.get(0).getStore().getTotalSeat() : 가게의 총 예약 가능 인원수
         * collect.get(date) 해당 날짜의 현재 예약한 인원수
         */
        for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
            if (collect.containsKey(date)) {
                list.add(new ReservationNumMonth(date, reservation.get(0).getStore().getTotalSeat() - collect.get(date)));
            } else {
                list.add(new ReservationNumMonth(date, reservation.get(0).getStore().getTotalSeat()));
            }
        }
        return list;
    }
    ...
    private int reservePeopleNumInDate(LocalDate date, StoreEntity store) {
        return reservationRepository.findByDateAndStoreId(date, store.getId()
                        , Arrays.asList(UNAPPROVED, APPROVED, VISITED)).stream()
    ...
    private static void validateChangeStatus(CustomUserDetails user, ReservationStatusDto.Request request, ReservationEntity reservation) {
        /**
         * 가게 주인(매니저)과 현재 로그인한 사용자가 동일한지 확인
         */
        ...
        /**
         * APPROVED, CANCELED 이외에 상태로 변경하려 하는지 확인
         */
        if (request.getReservationType() != APPROVED && request.getReservationType() != CANCELED) {
