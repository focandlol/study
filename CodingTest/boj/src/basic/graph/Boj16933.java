                ...
                if(x >= 1 && x<=n && y >= 1 && y<=m) {
                    if(cur[3] >= 1 && arr[x][y] == '1' && !visited[x][y][cur[2]+1]) {
                        if(cur[5] == 1) {
                            ...
                            q.add(new int[]{x, y, cur[2] + 1, cur[3] - 1, cur[4] + 1,cur[5] * -1});
                        }else{
                            q.add(new int[]{cur[0], cur[1], cur[2], cur[3], cur[4] + 1, cur[5]*-1});
                        }
                    }else if(arr[x][y] == '0' && !visited[x][y][cur[2]]) {
                        visited[x][y][cur[2]] = true;
                        q.add(new int[]{x,y,cur[2],cur[3],cur[4]+1,cur[5] * -1});
                    }
                }
            }
        }
